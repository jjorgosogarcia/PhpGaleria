...
class FileUpload {
    ...
    public function upload($email=null){
        if($this->error){
            return false;
        }
        ...
        if($email!=null){
            if (!self::directory($email)) {
                return mkdir("../controlUsuario/cuadros/" . $email) + move_uploaded_file($_FILES[$this->parametro]["tmp_name"], "../controlUsuario/cuadros/" . $email ."/" . $nombre . "." . $this->extension);
            } else {
                return move_uploaded_file($_FILES[$this->parametro]["tmp_name"], "../controlUsuario/cuadros/" . $email ."/" . $nombre . "." . $this->extension);
            }
        }
    ...
    private static function directory($email) {
        if (file_exists("../controlUsuario/cuadros/" . $email)) {
            return true;
        } else {
            return false;
        }
